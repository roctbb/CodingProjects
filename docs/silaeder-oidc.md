...
Приложение подключается к `https://lk.silaeder.ru` как конфиденциальный OpenID Connect-клиент. Вход доступен пользователям с ролями `student`, `teacher` и `admin`; внешняя роль `admin` преобразуется в локальную роль `teacher` и сама по себе не даёт административных прав в приложении. Если у локального аккаунта уже назначена роль `admin`, она сохраняется при входе и привязке ЛК, включая подтверждение по email.
...
- Имя и доступный email обновляются из `userinfo` при каждом входе. Роль также синхронизируется, кроме локальной роли `admin`: её назначение и снятие выполняются в приложении. При подтверждении привязки учитывается текущая локальная роль, а не роль на момент отправки письма.
